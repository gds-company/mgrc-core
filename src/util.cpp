...
const char * const BITCOIN_CONF_FILENAME = "mgrc.conf";
const char * const BITCOIN_PID_FILENAME = "mgrcd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "mgrc" is a composite category enabling all Master GRC-related debug output
                if(ptrCategory->count(std::string("mgrc"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\mgrcCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\mgrcCore
    // Mac: ~/Library/Application Support/mgrcCore
    // Unix: ~/.mgrccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "mgrcCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/mgrcCore";
#else
    // Unix
    return pathRet / ".mgrccore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty mgrc.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override mgrc.conf
